...
ACCOUNTS = [
    #{"email": os.getenv("EMAIL_1"), "senha": os.getenv("SENHA_1")},# juan meneghesso2
...
PASTAS = [
   ...
   # Pastas disponíveis na conta: marceliyeiri@gmail.com
  "2022",
  "2022/Pessoal",
  "2022/Tochatour",
  "GTA Seguros",
    ...
    try:
        ...
        try:
            ...
            for i in range(0, total_emails, batch_size):
                ...
                mail.store(batch_ids, '+FLAGS', '\\Deleted')
                mail.expunge()
...
def get_email_filename(email_msg):
    try:
        raw_subject = email_msg['Subject'] or "No Subject"
        ...
        with tqdm(total=len(PASTAS), desc=f"📁 Processado pastas:", position=0) as folder_pbar:
            for pasta in PASTAS:
                try:
                    ...
                    with tqdm(total=total_emails, desc=f"Backup ", position=1, leave=False) as email_pbar:
